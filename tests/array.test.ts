...
describe("ArraySL.duplicates", () => {
    ...
    describe("mode: 'all'", () => {
        ...
        test("should work with a custom accessor", () => {
            ...
            expect(result.map((i) => i.val).sort()).toEqual(["a", "b", "c", "e", "f"]);
        });
    });
...
describe("ArraySL.medianBy", () => {
    test("should find the single median in an odd-length array", () => {
        const arr = new ArraySL([{ val: 10 }, { val: 20 }, { val: 5 }]);
        const median = arr.medianBy((item) => item.val);
        expect(median.length).toBe(1);
        expect(median[0]).toEqual({ val: 10 });
    });

    test("should find the two medians in an even-length array", () => {
        const evenArr = new ArraySL([{ val: 10 }, { val: 20 }, { val: 5 }, { val: 30 }]);
        const medianEven = evenArr.medianBy((item) => item.val);
        expect(medianEven.length).toBe(2);
        expect(medianEven.map((i) => i.val)).toEqual([10, 20]);
    });

    test("should return an empty array for an empty input array", () => {
        const arr = new ArraySL<{ val: number }>();
        const median = arr.medianBy((item) => item.val);
        expect(median.length).toBe(0);
    });
});

describe("ArraySL.modeBy", () => {
    test("should find the mode in a simple case", () => {
        const arr = new ArraySL([{ val: "a" }, { val: "b" }, { val: "a" }]);
        const mode = arr.modeBy((item) => item.val);
    ...
    test("should return an empty array if there is no unique mode", () => {
        const arr = new ArraySL([1, 2, 3]);
        expect(arr.modeBy((i) => i).length).toBe(0);
    });
